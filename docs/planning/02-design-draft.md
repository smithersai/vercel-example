# Design Doc (Draft) — Smithers-on-Vercel Telegram Summary Bot

Status: draft v2 (2026-07-01). Builds on `docs/planning/01-prd.md` (requirements/AC IDs
referenced below) and `artifacts/smithering/research/design-art.md` (precedent survey).
...
granular detail the PRD intentionally leaves out. v2 addresses reviewer feedback on v1:
adds §3.5–3.7 (execution architecture, scheduling/catch-up loop, delivery retry/
idempotency) and §7.5 (coverage/e2e architecture), fixes the run.status "four"→"five"
count, and tightens D18's token-handoff auth flow.
...
cron/date arithmetic gets wrong; a maintained library push the DST edge cases onto code
...
## 3.5 Execution architecture (REQ-8, REQ-9) — serverless, Postgres-only, long-task path

The system's core mechanism — not covered elsewhere in this draft — is how a "run" moves
from a cron tick to a posted summary using only Vercel Functions, Vercel Cron, the
Sandbox long-task provider, and Postgres. This section is the implementer's starting
point for REQ-8/REQ-9; it is design-level (shape of the pipeline and its state machine),
...
nothing has been sent yet); (e) deliver via D24's per-chunk delivery ledger; (f) set
...
it can read from the `run` row and D24's delivery ledger, so every step must be safe to
re-run from scratch — this is simpler to reason about than a checkpoint/snapshot scheme
and needs no extra state beyond columns already in D2's `run` table plus D24's ledger.

**Decision D24 — see also D26 (D8 chunking's crash-resume interaction).**

## 3.6 Cron-tick → due-chat evaluation loop, including missed windows (AC-2.5)
...
## 3.7 Telegram delivery retry and per-chunk idempotency (AC-5.2, interaction with D2/D8)
...
## 6. Operator UI — layout and interaction

### 6.1 Runs list (home view)
...
## 7.5 Test/coverage architecture (REQ-10)
...
## 8. Open questions carried forward (not blocking this draft)
...
## 9. Explicit gaps / unverified claims

- This draft is based entirely on the PRD and the design-art research artifact; no new
  ...
  (§5 of that document), this draft treats them as accepted design precedent per that
